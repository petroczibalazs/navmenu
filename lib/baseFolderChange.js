...
*/
    ...
    fse.copySync(`${baseFolder}/tranzit/${renamedNewFolder}`, `${baseFolder}/element-groups/${renamedNewFolder}`);
    ...
    fs.rmdirSync(`${baseFolder}/tranzit/${renamedNewFolder}`, {recursive : true});
...
const actOnEvent = (curFolders) => {
    ...
    if(curFolders.length > prevFolders.length){
            ...
            if(newFolder.length == 0){
                newFolder = curFolders.filter( (value ,index) => curFolders.indexOf(value) != index)
            }
...
const getFolders = () => {
    ...
    fs.readdirSync(tranzitFolder).forEach((aName) => {
        if(aName.indexOf('_copied_') === -1 && aName.match(/libid_\d{3}_\d{3}/) && fs.statSync(tranzitFolder + '/' + aName).isDirectory()){
            curFolders.push(aName);
        }
    });
